...
export function Sidebar({
  ...
  const visible = NAV_ITEMS.filter((n) =>
    VIEW_ACCESS[n.id].includes(currentRole),
  );
    ...
    <aside className="w-60 flex-shrink-0 flex flex-col h-full bg-white border-r border-gray-200">
      ...
      >
        ...
        <div>
          ...
          <div
            className="text-blue-200/70 leading-tight"
            style={{ fontSize: "10px" }}
          >
            Modul PPO
          </div>
          ...
          return (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div
                  className={`text-[10px] font-medium leading-none mb-0.5 ${active ? "text-blue-400" : "text-gray-400"}`}
                >
                  {item.flow}
                </div>
                <div
                  className={`text-xs font-medium leading-tight truncate ${active ? "text-blue-700" : ""}`}
                >
                  {item.label}
                </div>
              </div>
