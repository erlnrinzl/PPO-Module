...
import {
  CheckSquare, Square, FileText, Send, X, Check, AlertCircle,
  Users, Search, Download, Mail, UserPlus, Plus, PackagePlus,
  HeartCrack, Stethoscope, ChevronRight, ArrowLeft, ClipboardList,
...
const FLOW_STEPS: Step[] = [
  ...
  { id: 2, label: 'Lengkapi Berkas', actor: 'SDM Satker/UE1', desc: 'Clearance BMN, Ikatan Dinas, dll.' },
...
type DokStatus = 'uploaded' | 'missing' | 'verified';
type Dokumen = { id: string; label: string; wajib: boolean; status: DokStatus };

const BASE_DOKUMEN: Dokumen[] = [
  { id: 'd1', label: 'Clearance BMN', wajib: true, status: 'uploaded' },
  { id: 'd2', label: 'Clearance Ikatan Dinas', wajib: true, status: 'missing' },
  { id: 'd3', label: 'Surat Pernyataan Tidak Pernah Hukdis Sedang/Berat', wajib: true, status: 'uploaded' },
  { id: 'd4', label: 'Data Tapem (TASPEN)', wajib: true, status: 'uploaded' },
  { id: 'd5', label: 'Data Profil Riwayat Jabatan (HRIS)', wajib: true, status: 'uploaded' },
  { id: 'd6', label: 'Data PPF (CLPB)', wajib: true, status: 'missing' },
  { id: 'd7', label: 'Fotokopi KTP yang masih berlaku', wajib: false, status: 'uploaded' },
  { id: 'd8', label: 'Pas Foto 3x4 Terbaru', wajib: false, status: 'missing' },
];

// ─── Kebutuhan berkas per jenis kasus ─────────────────────────────────────────

interface BerkasReq { id: string; nama: string; source: 'satu-kemenkeu' | 'upload'; }

const BERKAS_REQ_BUP: BerkasReq[] = [
...
const BERKAS_REQ_MENINGGAL: BerkasReq[] = [
...
const BERKAS_REQ_UZUR: BerkasReq[] = [
...
function getBerkasReqForJenis(jenisKasus: string): BerkasReq[] {
...
// Deterministic "status dokumen" per pegawai based on index
const KANDIDAT_WITH_STATUS = KANDIDAT_DATA.map((p, i) => ({
  ...p,
...
// Pegawai ids seeded per berkas — ambil dari KANDIDAT_WITH_STATUS (diinisialisasi setelah array tersebut)
const BERKAS_PEGAWAI_IDS: Record<string, string[]> = {
  u1: ['1', '2', '3', '4', '5', '6'],   // BUP Jan 2027 — sample 6 dari 35
  u2: ['7', '8', '9', '10'],             // BUP Feb 2027 — sample 4 dari 22
  u3: ['11', '12'],                      // Uzur — sample 2 dari 5
};

const MOCK_BERKAS: BerkasUsulan[] = [
  { id: 'u1', nomorUsulan: 'USP-2027-001', namaberkas: 'BUP Januari 2027',  jenis: 'BUP',  jumlahPegawai: 35, status: 'Draft',          tanggalDibuat: '02 Jan 2027', lengkap: 32, belumLengkap: 3, pegawaiIds: BERKAS_PEGAWAI_IDS.u1 },
  { id: 'u2', nomorUsulan: 'USP-2027-002', namaberkas: 'BUP Februari 2027', jenis: 'BUP',  jumlahPegawai: 22, status: 'Diajukan ke UE1', tanggalDibuat: '01 Feb 2027', lengkap: 22, belumLengkap: 0, pegawaiIds: BERKAS_PEGAWAI_IDS.u2 },
  { id: 'u3', nomorUsulan: 'USP-2027-003', namaberkas: 'Uzur Triwulan I',   jenis: 'Uzur', jumlahPegawai: 5,  status: 'Review UE1',    tanggalDibuat: '15 Mar 2027', lengkap: 5,  belumLengkap: 0, pegawaiIds: BERKAS_PEGAWAI_IDS.u3 },
];
...
function DokumenChecklist({ dokumen, editable }: { dokumen: Dokumen[]; editable: boolean }) {
  const [docs, setDocs] = useState(dokumen);
  const toggle = (id: string) => {
    if (!editable) return;
    setDocs(d => d.map(x => x.id === id ? { ...x, status: x.status === 'uploaded' ? 'missing' : 'uploaded' as DokStatus } : x));
  };
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-3">
        <p className="text-xs font-semibold text-gray-700">Kelengkapan Berkas ({uploaded}/{docs.length})</p>
        <div className="h-1.5 w-32 bg-gray-100 rounded-full overflow-hidden">
          <div className="h-full rounded-full bg-green-500 transition-all" style={{ width: `${(uploaded / docs.length) * 100}%` }} />
        </div>
      </div>
      <div className="space-y-2">
        {docs.map(doc => (
          ...
          >
            ...
            <p className="text-xs font-medium text-gray-700 flex-1">{doc.label}</p>
            <div className="flex items-center gap-1.5">
              {doc.wajib && <span className="text-[9px] bg-red-100 text-red-600 px-1.5 py-0.5 rounded-full font-semibold">Wajib</span>}
              {doc.status === 'verified'
                ? <span className="text-[9px] bg-blue-100 text-blue-700 px-1.5 py-0.5 rounded-full font-semibold">Verified</span>
                : doc.status === 'uploaded'
                ? <span className="text-[9px] bg-green-100 text-green-700 px-1.5 py-0.5 rounded-full font-semibold">Uploaded</span>
                : <span className="text-[9px] bg-red-100 text-red-600 px-1.5 py-0.5 rounded-full font-semibold">Belum</span>}
            </div>
          </div>
        ))}
      </div>
...
function CaseDetail({ pegawai, role, onClose, isArchived = false, statusDokumen = 'Lengkap', berkasStatus }: { pegawai: Pegawai; role: Role; onClose: () => void; isArchived?: boolean; statusDokumen?: 'Lengkap' | 'Belum Lengkap'; berkasStatus?: StatusUsulan }) {
  const [catatan, setCatatan] = useState(isArchived ? 'Proses pemberhentian telah selesai. SK Pemberhentian telah diterbitkan dan disampaikan kepada pegawai.' : '');
  ...
  const handleUpload = (id: string) => setManualUploads(prev => new Set([...prev, id]));
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-end sm:items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl w-full max-w-2xl shadow-2xl overflow-hidden max-h-[90vh] flex flex-col">
        <div className="px-6 py-4 flex items-start justify-between shrink-0" style={{ background: '#162D54' }}>
          <div>
            ...
            <div className="flex items-center gap-2 mt-1.5">
              <span className="text-[10px] text-white bg-white/20 px-2 py-0.5 rounded-full">{pegawai.nomorKasus}</span>
              <span className="text-[10px] text-white px-2 py-0.5 rounded-full font-semibold" style={{ background: getKasusColor(pegawai.jenisKasus) }}>
        ...
        <div className="overflow-y-auto flex-1 p-6 space-y-5">
          <div className="bg-gray-50 rounded-2xl p-4 overflow-x-auto">
            <p className="text-xs font-semibold text-gray-500 mb-3">Tahapan Proses (Flow 2)</p>
            <WorkflowStepper steps={FLOW_STEPS} currentStep={currentStep} />
          </div>
          <div className="grid grid-cols-3 gap-3">
            {[{ label: 'Tgl BUP', value: pegawai.tanggalBUP }, { label: 'Tgl Kasus', value: pegawai.tanggalKasus }, { label: 'Pangkat/Gol', value: pegawai.pangkatGolongan }].map(({ label, value }) => (
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <p className="text-sm font-semibold text-gray-800">Kelengkapan Berkas</p>
              <span className={`text-[10px] font-semibold px-2.5 py-1 rounded-full ${statusDokumen === 'Lengkap' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-600'}`}>
                {statusDokumen}
              </span>
            </div>
            ...
            <div className="grid grid-cols-4 gap-2">
              {[
                { label: 'Total',         value: berkasItems.length,   color: 'text-gray-700',  bg: 'bg-gray-50',   border: 'border-gray-200' },
                ...
                <div key={s.label} className={`rounded-xl border px-2 py-2 text-center ${s.bg} ${s.border}`}>
                  <p className="text-[9px] text-gray-400 uppercase tracking-wide font-semibold mb-0.5">{s.label}</p>
          ...
          <div className="border border-gray-200 rounded-xl overflow-hidden">
            ...
            >
              ...
              <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-blue-100 text-blue-700">
                {kemenkeuItems.length} Satu Kemenkeu · {uploadItems.length} Upload
              </span>
              <ChevronDown size={13} className={`text-gray-400 shrink-0 transition-transform duration-200 ${berkasExpanded ? 'rotate-180' : ''}`} />
            </button>
            {berkasExpanded && (
                ...
                {berkasItems.map((b, idx) => {
                  ...
                  return (
                    <div key={b.id} className="flex items-start gap-3 px-4 py-2.5 hover:bg-gray-50/80">
                      <span className="text-[10px] text-gray-400 font-semibold w-5 shrink-0 mt-0.5">{idx + 1}</span>
          ...
          <div>
            <p className="text-xs font-semibold text-gray-700 mb-2">Catatan / Komentar</p>
            <textarea
              value={catatan}
        ...
        <div className="px-6 py-4 border-t border-gray-100 flex items-center gap-3 shrink-0">
          {!isArchived && canEdit && (
            <>
              <label className="flex items-center gap-2 cursor-pointer mr-auto">
                ...
                <span className="text-xs text-gray-600">Berkas telah diperiksa dan lengkap</span>
              </label>
              <button disabled={!confirmed} className="flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-semibold text-white disabled:opacity-40" style={{ background: '#162D54' }}>
          ...
          {!isArchived && canVerify && (
            <>
              ...
              <button className="flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-semibold text-white ml-auto" style={{ background: '#162D54' }}>
                <Check size={13} /> Verifikasi & Teruskan
              </button>
            </>
          )}
          {!isArchived && !canEdit && !canVerify && <p className="text-xs text-gray-400 mr-auto">Anda hanya memiliki akses baca.</p>}
          <button onClick={onClose} className={`px-4 py-2 rounded-xl text-xs font-semibold text-gray-600 border border-gray-200 hover:bg-gray-50 ${isArchived ? 'ml-auto' : ''}`}>Tutup</button>
        </div>
      </div>
    </div>
  );
...
function SmartGroupingModal({ onClose, onConfirm }: {
  onClose: () => void;
  onConfirm: (groups: { name: string; count: number; jenis: 'BUP' | 'Uzur' | 'Meninggal' }[]) => void;
}) {
  const suggestions = [
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl w-full max-w-xl shadow-2xl overflow-hidden">
        <div className="px-6 py-4 flex items-center justify-between" style={{ background: '#162D54' }}>
          <div>
            <p className="text-white font-semibold">Smart Grouping — Buat Berkas Usulan</p>
            <p className="text-white/60 text-xs mt-0.5">Sistem menyarankan pengelompokan otomatis berdasarkan jenis kasus dan periode</p>
          </div>
        ...
        <div className="p-6 space-y-4">
          <div className="bg-blue-50 border border-blue-200 rounded-xl p-3 flex gap-2">
            ...
            <p className="text-xs text-blue-700">Pilih grouping yang ingin dibuat menjadi berkas usulan. Anda dapat memilih lebih dari satu.</p>
          </div>
          <div className="space-y-3">
            {suggestions.map((s, i) => (
              <div
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-gray-500 mt-0.5">→ {s.count} pegawai</p>
                  </div>
                  <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold text-white" style={{ background: getKasusColor(s.jenis) }}>
                    {s.jenis}
                  </span>
                </div>
              </div>
        ...
        <div className="px-6 py-4 border-t border-gray-100 flex items-center gap-3">
          ...
          <button
            onClick={() => { onConfirm(suggestions.filter((_, i) => chosen.has(i))); onClose(); }}
...
function ValidationModal({ berkas, onClose, onSubmit }: { berkas: BerkasUsulan; onClose: () => void; onSubmit: () => void }) {
      ...
      <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl overflow-hidden">
        ...
        <div className="p-6 space-y-4">
          <div className="bg-gray-50 rounded-xl p-4">
            <p className="text-sm font-semibold text-gray-800">{berkas.namaberkas}</p>
            <p className="text-xs text-gray-500 mt-0.5">{berkas.jumlahPegawai} Pegawai</p>
          </div>
          ...
          <div className="space-y-2.5">
            <div className="flex items-center justify-between bg-green-50 rounded-xl px-4 py-3">
              ...
              <span className="text-sm font-semibold text-green-700">{berkas.lengkap} pegawai</span>
            </div>
            {berkas.belumLengkap > 0 && (
              <div className="flex items-center justify-between bg-red-50 rounded-xl px-4 py-3">
                ...
                <span className="text-sm font-semibold text-red-700">{berkas.belumLengkap} pegawai</span>
              </div>
            )}
          </div>
          ...
          {!canSubmit ? (
            <div className="bg-red-50 border border-red-200 rounded-xl p-3 flex gap-2">
              ...
              <div>
                <p className="text-xs font-semibold text-red-700">Tidak dapat diajukan</p>
                <p className="text-xs text-red-600 mt-0.5">Berkas tidak dapat diajukan karena masih terdapat {berkas.belumLengkap} pegawai dengan dokumen belum lengkap atau clearance belum selesai.</p>
              </div>
            </div>
          ) : (
            <div className="bg-green-50 border border-green-200 rounded-xl p-3 flex gap-2">
              ...
              <div>
                <p className="text-xs font-semibold text-green-700">Siap diajukan</p>
                <p className="text-xs text-green-600 mt-0.5">Semua dokumen lengkap. Berkas dapat diajukan ke UE1.</p>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="px-6 py-4 border-t border-gray-100 flex items-center gap-3">
          <button onClick={onClose} className="px-4 py-2 rounded-xl text-xs font-semibold text-gray-600 border border-gray-200 hover:bg-gray-50">Tutup</button>
          {canSubmit && (
            <button
...
function PegawaiDokModal({ pegawai, onClose }: {
  pegawai: typeof KANDIDAT_WITH_STATUS[number];
  onClose: () => void;
}) {
  return (
    <div className="fixed inset-0 bg-black/40 flex items-end sm:items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden max-h-[85vh] flex flex-col">
        <div className="px-6 py-4 flex items-start justify-between shrink-0" style={{ background: '#162D54' }}>
          <div>
            <p className="text-white font-semibold">{pegawai.nama}</p>
            <p className="text-white/60 text-xs">{pegawai.nip} · {pegawai.jabatan}</p>
            <div className="flex items-center gap-2 mt-1.5">
              <span className="text-[10px] text-white bg-white/20 px-2 py-0.5 rounded-full">{pegawai.satker}</span>
              <span className="text-[10px] text-white px-2 py-0.5 rounded-full font-semibold" style={{ background: getKasusColor(pegawai.jenisKasus) }}>
                {getKasusLabel(pegawai.jenisKasus)}
              </span>
            </div>
        ...
        <div className="overflow-y-auto flex-1 p-6">
          <p className="text-sm font-semibold text-gray-700 mb-4">Kelengkapan Dokumen Pegawai</p>
...
function BerkasDetailPage({ berkas, onBack, role }: {
  berkas: BerkasUsulan;
  onBack: () => void;
  role: Role;
}) {
  const [selectedPegawai, setSelectedPegawai] = useState<typeof KANDIDAT_WITH_STATUS[number] | null>(null);
  const [pegawaiSearch, setPegawaiSearch] = useState('');

  const currentIdx = LIFECYCLE.findIndex(l => l.status === berkas.status);
  const resolvedPegawai = (berkas.pegawaiIds || [])
    .map(id => KANDIDAT_WITH_STATUS.find(p => p.id === id))
    .filter((p): p is (typeof KANDIDAT_WITH_STATUS)[number] => p !== undefined);

  const filteredPegawai = (resolvedPegawai || []).filter(p =>
    !pegawaiSearch ||
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between gap-3 flex-wrap">
              <div>
                ...
                <p className="text-white/70 text-xs mt-0.5">{berkas.nomorUsulan} · Dibuat {berkas.tanggalDibuat}</p>
              </div>
              <div className="flex items-center gap-2 flex-wrap">
                <span className="text-[10px] text-white px-2 py-0.5 rounded-full font-semibold" style={{ background: getKasusColor(berkas.jenis) }}>{berkas.jenis}</span>
        ...
        <div className="grid grid-cols-3 divide-x divide-gray-100 border-t border-gray-100">
          <div className="px-4 py-3 text-center">
            <p className="text-[10px] text-gray-400 uppercase tracking-wide flex items-center justify-center gap-1"><Users size={10} /> Total Pegawai</p>
            <p className="text-xl font-bold text-gray-800 mt-0.5">{totalDisplay}</p>
          </div>
          <div className="px-4 py-3 text-center">
            <p className="text-[10px] text-green-600 uppercase tracking-wide flex items-center justify-center gap-1"><Check size={10} /> Dokumen Lengkap</p>
            <p className="text-xl font-bold text-green-700 mt-0.5">{berkas.lengkap}</p>
          </div>
          <div className="px-4 py-3 text-center">
            <p className={`text-[10px] uppercase tracking-wide flex items-center justify-center gap-1 ${berkas.belumLengkap > 0 ? 'text-red-600' : 'text-gray-400'}`}>
              <AlertCircle size={10} /> Belum Lengkap
            </p>
            <p className={`text-xl font-bold mt-0.5 ${berkas.belumLengkap > 0 ? 'text-red-700' : 'text-gray-400'}`}>{berkas.belumLengkap}</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
        ...
        <div className="relative">
          ...
          <div
            ...
            style={{ height: currentIdx > 0 ? `calc(${(currentIdx / (LIFECYCLE.length - 1)) * 100}%)` : '0%' }}
          />
            ...
            {LIFECYCLE.map((l, i) => {
              ...
              return (
                <div key={l.status} className="flex items-start gap-4 relative">
                  <div className={`w-7 h-7 rounded-full flex items-center justify-center shrink-0 z-10 transition-all ${isActive ? 'bg-blue-500 ring-4 ring-blue-100' : isPast ? 'bg-blue-500' : 'bg-gray-200'}`}>
                    {isPast || isActive
        ...
        <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between flex-wrap gap-3">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">
              Menampilkan {sampleShown} data ({totalDisplay - sampleShown > 0 ? `+${totalDisplay - sampleShown} lainnya` : 'semua'} total {totalDisplay} pegawai) · Klik baris untuk cek dokumen
            </p>
          </div>
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-100">
                ...
                <th className="px-4 py-3 text-left text-gray-500 font-semibold">Nama / NIP</th>
                <th className="px-4 py-3 text-left text-gray-500 font-semibold">Jabatan / Satker</th>
                <th className="px-4 py-3 text-left text-gray-500 font-semibold whitespace-nowrap">Jenis Kasus</th>
                <th className="px-4 py-3 text-left text-gray-500 font-semibold whitespace-nowrap">TMT BUP</th>
                <th className="px-4 py-3 text-left text-gray-500 font-semibold whitespace-nowrap">Status Dokumen</th>
                ...
                >
                  ...
                  <td className="px-4 py-3 whitespace-nowrap">
                    <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-semibold ${p.statusDokumen === 'Lengkap' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                      {p.statusDokumen === 'Lengkap' ? <Check size={10} /> : <AlertCircle size={10} />}
                      {p.statusDokumen}
                    </span>
                  </td>
                  <td className="px-4 py-3">
        ...
        {totalDisplay > sampleShown && (
          <div className="px-4 py-3 border-t border-gray-100 bg-gray-50 text-center">
            <p className="text-xs text-gray-400">
              Menampilkan {sampleShown} dari {totalDisplay} pegawai. Data lengkap tersedia setelah integrasi sistem kepegawaian.
            </p>
          </div>
        )}
      </div>
...
type JenisLaporan = 'Meninggal' | 'Uzur';

interface TambahKasusModalProps {
  onClose: () => void;
  onSave: (entry: typeof KANDIDAT_WITH_STATUS[number]) => void;
}
      ...
      <div className="bg-white rounded-2xl w-full max-w-xl shadow-2xl overflow-hidden flex flex-col max-h-[92vh]">
        ...
        <div className="px-6 py-4 flex items-start justify-between shrink-0" style={{ background: '#162D54' }}>
          <div>
            ...
            <p className="text-white/60 text-xs mt-0.5">Unit SDM Satker · Flow 2</p>
          </div>
          <button onClick={onClose} className="text-white/60 hover:text-white"><X size={20} /></button>
        </div>
        ...
        <div className="px-6 pt-5 pb-0 shrink-0">
          <p className="text-xs font-semibold text-gray-600 mb-3">Jenis Kasus</p>
          <div className="grid grid-cols-2 gap-3">
            {(['Meninggal', 'Uzur'] as JenisLaporan[]).map(j => (
              <button
                key={j}
                onClick={() => { setJenis(j); setStep(1); setErrors({}); }}
                className={`flex items-center gap-3 p-4 rounded-xl border-2 transition-all text-left ${
                  jenis === j
                    ? j === 'Meninggal' ? 'border-gray-700 bg-gray-50' : 'border-amber-500 bg-amber-50'
                    : 'border-gray-200 hover:border-gray-300 bg-white'
                }`}
              >
                <div className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 ${jenis === j ? (j === 'Meninggal' ? 'bg-gray-700' : 'bg-amber-500') : 'bg-gray-100'}`}>
                  {j === 'Meninggal'
                    ? <HeartCrack size={18} className={jenis === j ? 'text-white' : 'text-gray-400'} />
                    : <Stethoscope size={18} className={jenis === j ? 'text-white' : 'text-gray-400'} />}
                </div>
                <div>
                  <p className={`text-sm font-semibold ${jenis === j ? 'text-gray-800' : 'text-gray-500'}`}>{j === 'Meninggal' ? 'Meninggal Dunia' : 'Uzur / Sakit'}</p>
                  <p className="text-[10px] text-gray-400 mt-0.5 leading-tight">
                    {j === 'Meninggal' ? 'Pegawai meninggal dalam dinas' : 'Tidak mampu menjalankan tugas'}
                  </p>
                </div>
              </button>
            ))}
            ...
            {['Data Pegawai', 'Detail Kasus'].map((s, i) => {
              ...
              return (
                <div key={s} className="flex items-center gap-2 flex-1">
                  <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold shrink-0 ${done ? 'bg-green-500 text-white' : active ? 'text-white' : 'bg-gray-200 text-gray-400'}`}
            ...
            <>
              ...
              {resolvedPegawai && (
                <div className="bg-green-50 border border-green-200 rounded-xl p-3 flex items-center gap-3">
                  ...
                  <button onClick={() => { setResolvedPegawai(null); setManualNama(''); setManualNip(''); setSatker(''); setJabatan(''); setNipQuery(''); }} className="text-gray-400 hover:text-gray-600">
                    <X size={14} />
                  </button>
                </div>
              )}
          ...
          {step === 2 && (
            <>
              ...
              <div className="bg-gray-50 rounded-xl p-3 flex items-center gap-3">
                <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${jenis === 'Meninggal' ? 'bg-gray-700' : 'bg-amber-500'}`}>
                  {jenis === 'Meninggal'
                ...
                <span className="ml-auto text-[10px] px-2 py-0.5 rounded-full font-semibold text-white" style={{ background: getKasusColor(jenis) }}>{jenis}</span>
              </div>
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-3">
                    <Field label="Tanggal Meninggal *" error={errors.tanggal}>
                      <input type="date" value={tanggalMeninggal} onChange={e => setTanggalMeninggal(e.target.value)} className={inputCls(errors.tanggal)} />
                    </Field>
                    <Field label="Tempat Meninggal">
                      <input value={tempatMeninggal} onChange={e => setTempatMeninggal(e.target.value)} placeholder="RS / kota..." className={inputCls()} />
                    </Field>
                  </div>
                  <Field label="No. Surat Keterangan Kematian *" error={errors.nomSurat}>
                    <input value={nomorSuratKematian} onChange={e => setNomorSuratKematian(e.target.value)} placeholder="Misal: 474/SKK/IV/2027" className={inputCls(errors.nomSurat)} />
                  </Field>
                  <Field label="Penyebab Kematian *" error={errors.penyebab}>
                    <input value={penyebabKematian} onChange={e => setPenyebabKematian(e.target.value)} placeholder="Sakit / kecelakaan / dll" className={inputCls(errors.penyebab)} />
                  </Field>
                  <Field label="Keterangan Tambahan">
                    <textarea value={keterangan} onChange={e => setKeterangan(e.target.value)} placeholder="Informasi tambahan yang relevan..." rows={3} className={`${inputCls()} resize-none`} />
                  </Field>
                </div>
              ) : (
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-3">
                    <Field label="Tanggal Diagnosis *" error={errors.tanggal}>
                      <input type="date" value={tanggalDiagnosis} onChange={e => setTanggalDiagnosis(e.target.value)} className={inputCls(errors.tanggal)} />
                    </Field>
                    <Field label="Institusi Medis">
                      <input value={institusiMedis} onChange={e => setInstitusiMedis(e.target.value)} placeholder="Nama RS / klinik" className={inputCls()} />
                    </Field>
                  </div>
                  <Field label="Kondisi / Jenis Uzur *" error={errors.kondisi}>
                    <select value={kondisiUzur} onChange={e => setKondisiUzur(e.target.value)} className={inputCls(errors.kondisi)}>
                      <option value="">Pilih kondisi uzur...</option>
                      {PENYEBAB_UZUR.map(p => <option key={p} value={p}>{p}</option>)}
                    </select>
                  </Field>
                  <Field label="No. Surat Keterangan Dokter *" error={errors.nomSurat}>
                    <input value={nomorSuratDokter} onChange={e => setNomorSuratDokter(e.target.value)} placeholder="Misal: SKD/2027/IV/001" className={inputCls(errors.nomSurat)} />
                  </Field>
                  <Field label="Keterangan Tambahan">
                    <textarea value={keterangan} onChange={e => setKeterangan(e.target.value)} placeholder="Kondisi terkini, riwayat pengobatan, dll..." rows={3} className={`${inputCls()} resize-none`} />
                  </Field>
                </div>
              )}

              <div className={`rounded-xl p-3 flex gap-2 ${jenis === 'Meninggal' ? 'bg-gray-50 border border-gray-200' : 'bg-amber-50 border border-amber-200'}`}>
                <AlertCircle size={13} className={`shrink-0 mt-0.5 ${jenis === 'Meninggal' ? 'text-gray-500' : 'text-amber-600'}`} />
                <p className={`text-xs ${jenis === 'Meninggal' ? 'text-gray-600' : 'text-amber-700'}`}>
                  {jenis === 'Meninggal'
          ...
          {step === 1 ? (
            ...
            <>
              ...
              <button
                onClick={handleSave}
                className="flex items-center gap-2 px-5 py-2 rounded-xl text-xs font-semibold text-white ml-auto"
                style={{ background: jenis === 'Meninggal' ? '#374151' : '#D97706' }}
              >
                <Check size={14} />
...
export function FlowBUP({ role }: { role: Role }) {
  ...
  const [mainTab, setMainTab]         = useState<'kandidat' | 'berkas' | 'arsip'>('kandidat');
  const [selectedPegawai, setSelectedPegawai] = useState<typeof KANDIDAT_WITH_STATUS[number] | null>(null);
  const [activeBerkas, setActiveBerkas]       = useState<BerkasUsulan | null>(null);
  const [showGrouping, setShowGrouping]       = useState(false);
  const [showValidation, setShowValidation]   = useState<BerkasUsulan | null>(null);
  ...
  if (activeBerkas) {
    return (
      <div className="p-6">
        <BerkasDetailPage berkas={activeBerkas} onBack={() => setActiveBerkas(null)} role={role} />
      </div>
    );
  }

  const handleTambahKasus = (entry: typeof KANDIDAT_WITH_STATUS[number]) => {
  ...
  const filtered = (kandidatList || []).filter(p => {
    ...
    if (satkerFilter && !p.satker?.toLowerCase().includes(satkerFilter.toLowerCase())) return false;
    if (search && !p.nama?.toLowerCase().includes(search.toLowerCase()) && !p.nip?.includes(search)) return false;
    return true;
  });

  const allChecked = filtered.length > 0 && filtered.every((p: any) => checkedIds.has(p.id));
  const toggleAll  = () => setCheckedIds(allChecked ? new Set() : new Set(filtered.map(p => p.id)));
  ...
  const handleGroupConfirm = (groups: { name: string; count: number; jenis: 'BUP' | 'Uzur' | 'Meninggal' }[]) => {
  ...
  const handleSubmitBerkas = (id: string) => {
    setBerkaslist(prev => prev.map(b => b.id === id ? { ...b, status: 'Diajukan ke UE1' as StatusUsulan } : b));
  };
  ...
  return (
    <div className="p-6 space-y-5">
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-2xl p-4 flex gap-3">
        ...
        <div>
          <p className="text-sm font-semibold text-blue-800">Flow 2: Usulan Pemberhentian — BUP, Meninggal, Uzur</p>
          <p className="text-xs text-blue-600 mt-0.5">
            Kelola kandidat pemberhentian dari hasil monitoring (Flow 1) dan buat berkas usulan untuk diajukan ke hierarki yang lebih tinggi.
          </p>
        </div>
      </div>
      ...
      <div className="flex gap-1 bg-gray-100 rounded-xl p-1 w-fit">
        {(['kandidat', 'berkas', 'arsip'] as const).map(tab => {
          const berkasAktif = (berkaslist || []).filter(b => b && b.status !== 'SK Terbit');
          const berkasArsip = (berkaslist || []).filter(b => b && b.status === 'SK Terbit');
          return (
            <button
              key={tab}
            ...
            >
              {tab === 'kandidat' ? 'Kandidat Pemberhentian' : tab === 'berkas' ? 'Berkas Usulan' : 'Arsip'}
              {tab === 'berkas' && (
        ...
        <>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-4">
            <div className="flex items-center justify-between">
              ...
              <button
                onClick={() => { setJenisFilter(''); setStatusFilter(''); setSatkerFilter(''); setPeriodeFilter(''); setSearch(''); }}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
              <div>
                <label className="text-xs font-semibold text-gray-600 mb-1.5 block">Jenis Kasus</label>
                <select
                  value={jenisFilter}
              ...
              <div>
                <label className="text-xs font-semibold text-gray-600 mb-1.5 block">Status Dokumen</label>
                <select
                  value={statusFilter}
              ...
              <div>
                <label className="text-xs font-semibold text-gray-600 mb-1.5 block">Periode BUP</label>
                <input
                  value={periodeFilter}
          ...
          {checkedIds.size > 0 && (
            <div className="bg-[#162D54] rounded-2xl p-4 flex items-center justify-between flex-wrap gap-3">
              <p className="text-sm font-semibold text-white">{checkedIds.size} pegawai dipilih</p>
              <div className="flex items-center gap-2 flex-wrap">
                <button
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between flex-wrap gap-3">
              <div>
                <h3 className="font-semibold text-gray-800">Daftar Kandidat Pemberhentian</h3>
                <p className="text-xs text-gray-500">{filtered.length} dari {kandidatList.length} pegawai</p>
              </div>
              <div className="flex items-center gap-2 flex-wrap">
                <div className="flex items-center gap-2 bg-gray-50 border border-gray-200 rounded-xl px-3 py-2">
              ...
              <table className="w-full text-xs">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-100">
                    ...
                    <th className="px-4 py-3 text-left text-gray-500 font-semibold whitespace-nowrap">Jenis Kasus</th>
                    <th className="px-4 py-3 text-left text-gray-500 font-semibold whitespace-nowrap">TMT</th>
                ...
                <tbody className="divide-y divide-gray-50">
                  {filtered.length === 0 ? (
                    ...
                    <tr key={p.id} className={`hover:bg-blue-50/30 transition-colors ${checkedIds.has(p.id) ? 'bg-blue-50/50' : ''}`}>
                      <td className="px-4 py-3">
                        <input type="checkbox" checked={checkedIds.has(p.id)} onChange={() => toggleOne(p.id)} className="rounded" />
                      </td>
                      <td className="px-4 py-3">
                        <p className="font-semibold text-gray-800">{p.nama}</p>
                        <p className="text-gray-400 mt-0.5">{p.nip}</p>
                      ...
                      <td className="px-4 py-3 text-gray-600 whitespace-nowrap">{p.tanggalBUP}</td>
                      <td className="px-4 py-3 whitespace-nowrap">
                        <span className="font-semibold" style={{ color: getSisaBulanColor(p.sisaBulan) }}>
            ...
            <div className="px-4 py-3 border-t border-gray-100 flex items-center justify-between">
              <p className="text-xs text-gray-500">Menampilkan {filtered.length} dari {kandidatList.length} pegawai</p>
              {checkedIds.size > 0 && (
                <button
                  onClick={() => setShowGrouping(true)}
                  className="flex items-center gap-1.5 text-xs font-semibold text-blue-600 hover:text-blue-700"
                >
                  <PackagePlus size={13} /> Buat berkas dari {checkedIds.size} terpilih
                </button>
              )}
            </div>
          </div>
      ...
      {mainTab === 'berkas' && (
        <>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-4">
            <div className="flex items-center justify-between">
              <p className="text-sm font-semibold text-gray-800">Filter Berkas Usulan</p>
              <button
                onClick={() => { setBerkasSearch(''); setBerkasJenisFilter(''); setBerkasStatusFilter(''); setBerkasTanggalFilter(''); }}
                className="text-xs text-blue-600 hover:text-blue-700 font-medium"
              >
                Reset
              </button>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
              ...
              <div>
                <label className="text-xs font-semibold text-gray-600 mb-1.5 block">Jenis</label>
                <select
              ...
              <div>
                <label className="text-xs font-semibold text-gray-600 mb-1.5 block">Tanggal Dibuat</label>
                <input
                  type="date"
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 mt-0.5">
                {(berkaslist || []).filter(b => {
                  ...
                  if (berkasSearch && !b.nomorUsulan?.toLowerCase().includes(berkasSearch.toLowerCase()) && !b.namaberkas?.toLowerCase().includes(berkasSearch.toLowerCase())) return false;
                  if (berkasJenisFilter && b.jenis !== berkasJenisFilter) return false;
                  if (berkasStatusFilter && b.status !== berkasStatusFilter) return false;
                  return true;
                }).length} berkas aktif
              </p>
            </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full text-xs">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-100">
                    <th className="px-4 py-3 text-left text-gray-500 font-semibold whitespace-nowrap">Nomor Usulan</th>
                    <th className="px-4 py-3 text-left text-gray-500 font-semibold">Nama Berkas</th>
                    <th className="px-4 py-3 text-left text-gray-500 font-semibold">Jenis</th>
                ...
                <tbody className="divide-y divide-gray-50">
                  {berkaslist.filter(b => {
                    ...
                    <tr>
                      <td colSpan={6} className="py-12 text-center text-gray-400">
                        ...
                        {berkaslist.filter(b => b.status !== 'SK Terbit').length === 0 && (
                          <button onClick={() => setShowGrouping(true)} className="mt-3 text-xs text-blue-600 hover:text-blue-700 font-medium">
                            Buat berkas pertama
                          </button>
                        )}
                      </td>
                    ...
                    <tr key={b.id} className="hover:bg-blue-50/30 transition-colors cursor-pointer" onClick={() => setActiveBerkas(b)}>
                      <td className="px-4 py-3">
                        <p className="font-semibold text-gray-800">{b.nomorUsulan}</p>
                        <p className="text-gray-400 mt-0.5">{b.tanggalDibuat}</p>
                      </td>
                      <td className="px-4 py-3 text-gray-700">{b.namaberkas}</td>
                      <td className="px-4 py-3 whitespace-nowrap">
                        <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold text-white" style={{ background: getKasusColor(b.jenis) }}>
                          {b.jenis}
                        </span>
                      </td>
                      <td className="px-4 py-3">
                        <div className="flex flex-col gap-0.5">
                          <span className="font-semibold text-gray-800">{b.jumlahPegawai} pegawai</span>
                          <div className="flex items-center gap-2">
                            <span className="text-[9px] text-green-600 font-semibold">✓ {b.lengkap} lengkap</span>
                            {b.belumLengkap > 0 && (
                              <span className="text-[9px] text-red-600 font-semibold">⚠ {b.belumLengkap} belum</span>
                            )}
                          </div>
                        </div>
                      </td>
                      <td className="px-4 py-3 whitespace-nowrap">
                        <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold text-white" style={{ background: statusUsulanColor(b.status) }}>
                          {b.status}
                        </span>
                      </td>
                      <td className="px-4 py-3">
        ...
        <>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-4">
            <div className="flex items-center justify-between">
              ...
              <button
                onClick={() => { setBerkasSearch(''); setBerkasJenisFilter(''); setBerkasTanggalFilter(''); }}
                className="text-xs text-blue-600 hover:text-blue-700 font-medium"
              >
                Reset
              </button>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
              ...
              <div>
                <label className="text-xs font-semibold text-gray-600 mb-1.5 block">Tanggal SK Terbit</label>
                <input
                  type="date"
                  value={berkasTanggalFilter}
                  onChange={e => setBerkasTanggalFilter(e.target.value)}
              ...
              <table className="w-full text-xs">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-100">
                    ...
                    <th className="px-4 py-3 text-left text-gray-500 font-semibold">Jenis</th>
                    <th className="px-4 py-3 text-left text-gray-500 font-semibold whitespace-nowrap">Jumlah Pegawai</th>
                    <th className="px-4 py-3 text-left text-gray-500 font-semibold whitespace-nowrap">Tanggal Dibuat</th>
                    <th className="px-4 py-3 text-left text-gray-500 font-semibold">Status</th>
                    <th className="px-4 py-3 text-left text-gray-500 font-semibold">Aksi</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-50">
                  {(berkaslist || []).filter(b => {
                    ...
                    <tr key={b.id} className="hover:bg-green-50/30 transition-colors cursor-pointer" onClick={() => setActiveBerkas(b)}>
                      <td className="px-4 py-3">
                        <p className="font-semibold text-gray-800">{b.nomorUsulan}</p>
                        <p className="text-gray-400 mt-0.5">{b.tanggalDibuat}</p>
                      </td>
                      ...
                      <td className="px-4 py-3 whitespace-nowrap text-gray-600">{b.tanggalDibuat}</td>
                      <td className="px-4 py-3 whitespace-nowrap">
                      ...
                      <td className="px-4 py-3">
                        <button
                          onClick={e => { e.stopPropagation(); setActiveBerkas(b); }}
            ...
            {berkaslist.filter(b => b.status === 'SK Terbit').length > 0 && (
              <div className="px-4 py-3 border-t border-gray-100">
                <p className="text-xs text-gray-500">
                  Menampilkan {(berkaslist || []).filter(b => {
                    if (!b) return false;
                    if (b.status !== 'SK Terbit') return false;
                    if (berkasSearch && !b.nomorUsulan?.toLowerCase().includes(berkasSearch.toLowerCase()) && !b.namaberkas?.toLowerCase().includes(berkasSearch.toLowerCase())) return false;
                    if (berkasJenisFilter && b.jenis !== berkasJenisFilter) return false;
                    return true;
                  }).length} dari {berkaslist.filter(b => b.status === 'SK Terbit').length} berkas arsip
                </p>
              </div>
            )}
          </div>
      ...
      {/* ── Modals ──────────────────────────────────────────────────────────── */}
      {selectedPegawai  && <CaseDetail    pegawai={selectedPegawai}   role={role} statusDokumen={selectedPegawai.statusDokumen} onClose={() => setSelectedPegawai(null)} />}
      {showGrouping     && <SmartGroupingModal onClose={() => setShowGrouping(false)} onConfirm={handleGroupConfirm} />}
      {showTambahKasus  && <TambahKasusModal  onClose={() => setShowTambahKasus(false)} onSave={handleTambahKasus} />}
      {showValidation   && (
        <ValidationModal
